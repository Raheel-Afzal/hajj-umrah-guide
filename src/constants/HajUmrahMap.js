import {Hajj, Umrah} from '../Enums';
import {IMAGES} from './Images';

export const HajUmrahMap = {
  [Hajj.QIRAN]: [
    {
      step: 'Enter into Ihram',
      image: IMAGES.enter_into_ihram,
    },
    {
      step: 'Perform Tawaf',
      image: IMAGES.perform_tawaf,
    },
    {
      step: 'Perform Saey',
      image: IMAGES.perform_saey,
    },
    {
      step: 'Day at Arafat',
      image: IMAGES.day_at_arafat,
    },
    {
      step: 'Night at Muzdalifah',
      image: IMAGES.night_at_muzdalifah,
    },
    {
      step: 'Devil Stoning in Mina',
      image: IMAGES.devil_stoning,
    },
    {
      step: 'Animal Sacrifice',
      image: IMAGES.animal_sacrifice,
    },
    {
      step: 'Hair Trimimg',
      image: IMAGES.hair_trim,
    },
    {
      step: 'Return to Kabah',
      image: IMAGES.return_to_kabah,
    },
    {
      step: 'Return to Devil Stoning',
      image: IMAGES.devil_stoning,
    },
    {
      step: 'Farewell Tawaf',
      image: IMAGES.fare_well_tawaf,
    },
  ],
  [Hajj.IFRAD]: [
    {
      step: 'Enter into Ihram',
      image: IMAGES.enter_into_ihram,
    },
    {
      step: 'Perform Tawaf',
      image: IMAGES.perform_tawaf,
    },
    {
      step: 'Perform Saey',
      image: IMAGES.perform_saey,
    },
    {
      step: 'Devil Stoning in Mina',
      image: IMAGES.devil_stoning,
    },
    {
      step: 'Animal Sacrifice',
      image: IMAGES.animal_sacrifice,
    },
    {
      step: 'Hair Trimimg',
      image: IMAGES.hair_trim,
    },
    {
      step: 'Return to Devil Stoning',
      image: IMAGES.devil_stoning,
    },
    {
      step: 'Farewell Tawaf',
      image: IMAGES.fare_well_tawaf,
    },
  ],
  [Hajj.TAMATTU]: [
    {
      step: 'Enter into Ihram',
      image: IMAGES.enter_into_ihram,
    },
    {
      step: 'Perform Tawaf',
      image: IMAGES.perform_tawaf,
    },
    {
      step: 'Perform Saey',
      image: IMAGES.perform_saey,
    },
    {
      step: 'Devil Stoning in Mina',
      image: IMAGES.devil_stoning,
    },
    {
      step: 'Animal Sacrifice',
      image: IMAGES.animal_sacrifice,
    },
    {
      step: 'Hair Trimimg',
      image: IMAGES.hair_trim,
    },
    {
      step: 'Farewell Tawaf',
      image: IMAGES.fare_well_tawaf,
    },
  ],
  [Umrah.MUDARAF]: [
    {
      step: 'Enter into Ihram',
      image: IMAGES.enter_into_ihram,
    },
    {
      step: 'Perform Tawaf',
      image: IMAGES.perform_tawaf,
    },
    {
      step: 'Perform Saey',
      image: IMAGES.perform_saey,
    },
    {
      step: 'Halq or Taqsir',
      image: IMAGES.hair_trim,
    }
   

  ],
  [Umrah.TAMMATU]: [
    {
      step: 'Enter into Ihram',
      image: IMAGES.enter_into_ihram,
    },
    {
      step: 'Perform Tawaf',
      image: IMAGES.perform_tawaf,
    },
    {
      step: 'Perform Saey',
      image: IMAGES.perform_saey,
    },
    {
      step: 'Halq or Taqsir',
      image: IMAGES.hair_trim,
    }
    
  ],
};
